...
export default function PointeurBadge({ pointeurUserId, pointeurNom, profils, onSelect, variant = 'circle' }) {
  ...
  const changeFlash = useFlashOnChange(pointeurUserId);
    ...
    <span className="relative inline-block" ref={ref}>
      {variant === 'circle' ? (
        <button type="button" onClick={(e) => { e.stopPropagation(); setOpen(o => !o); }} title={titre} aria-label={titre}
          className={`flex items-center justify-center h-3.5 w-3.5 rounded-full text-[8px] font-bold leading-none transition-colors active:scale-90
            ${initiale ? 'text-white' : 'border border-dashed border-gray-300 hover:border-gray-400'}
            ${changeFlash ? 'animate-pop' : ''}`}
      ...
      {open && (
        <div
          ...
          className="absolute z-30 top-full right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg py-1 w-44 max-h-56 overflow-y-auto animate-popoverIn"
        >
